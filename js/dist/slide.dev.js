...
function nextMove() {
  ...
  if (currSlide <= maxSlide) {
    // 슬라이드를 이동시키기 위한 offset 계산
    var _offset5 = slideWidth * currSlide; // 각 슬라이드 아이템의 left에 offset 적용


    slideItems.forEach(function (i) {
      i.setAttribute("style", "left: ".concat(-_offset5, "px"));
    });
  } else {
    // 무한 슬라이드 기능 - currSlide 값만 변경해줘도 되지만 시각적으로 자연스럽게 하기 위해 아래 코드 작성
    currSlide = 0;

    var _offset6 = slideWidth * currSlide;

    slideItems.forEach(function (i) {
      i.setAttribute("style", "transition: ".concat(0, "s; left: ", -_offset6, "px"));
    });
    currSlide++;
    _offset6 = slideWidth * currSlide; // 각 슬라이드 아이템의 left에 offset 적용
    // setTimeout을 사용하는 이유는 비동기 처리를 이용해 transition이 제대로 적용되게 하기 위함

    setTimeout(function () {
      // 각 슬라이드 아이템의 left에 offset 적용
      slideItems.forEach(function (i) {
        i.setAttribute("style", "transition: ".concat(2, "s; left: ", -_offset6, "px"));
      });
    }, 0);
  }
}
...
function prevMove() {
  ...
  if (currSlide > 0) {
    // 슬라이드를 이동시키기 위한 offset 계산
    var _offset7 = slideWidth * currSlide; // 각 슬라이드 아이템의 left에 offset 적용


    slideItems.forEach(function (i) {
      i.setAttribute("style", "left: ".concat(-_offset7, "px"));
    });
  } else {
    // 무한 슬라이드 기능 - currSlide 값만 변경해줘도 되지만 시각적으로 자연스럽게 하기 위해 아래 코드 작성
    currSlide = maxSlide + 1;

    var _offset8 = slideWidth * currSlide; // 각 슬라이드 아이템의 left에 offset 적용


    slideItems.forEach(function (i) {
      i.setAttribute("style", "transition: ".concat(0, "s; left: ", -_offset8, "px"));
    });
    currSlide--;
    _offset8 = slideWidth * currSlide;
    setTimeout(function () {
      // 각 슬라이드 아이템의 left에 offset 적용
      slideItems.forEach(function (i) {
        // i.setAttribute("style", `transition: ${0}s; left: ${-offset}px`);
        i.setAttribute("style", "transition: ".concat(2, "s; left: ", -_offset8, "px"));
      });
    }, 0);
  }
} // 버튼 엘리먼트에 클릭 이벤트 추가하기
